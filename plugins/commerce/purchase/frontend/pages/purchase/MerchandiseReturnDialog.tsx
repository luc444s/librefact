...
>(function MerchandiseReturnDialog({ products }, ref) {
  ...
  const dispatchCylinderOptionsByProduct = new Map<string, Array<{ value: string; label: string; keywords?: string[] }>>();
  const dispatchCylinderProductById = new Map<string, string>();
  (dispatchesQuery.data?.items ?? []).forEach((dispatch) => {
    dispatch.cylinders.forEach((cylinder) => {
      if (!cylinder.product_id || dispatchCylinderProductById.has(cylinder.cylinder_id)) return;
      dispatchCylinderProductById.set(cylinder.cylinder_id, cylinder.product_id);
      const options = dispatchCylinderOptionsByProduct.get(cylinder.product_id) ?? [];
      options.push({
        value: cylinder.cylinder_id,
        label: `${cylinder.serial ?? cylinder.cylinder_id.slice(0, 8)} · ${dispatch.dispatch_date}`,
        keywords: [cylinder.serial ?? "", dispatch.dispatch_date, productLabel.get(cylinder.product_id) ?? cylinder.product_id],
      });
      dispatchCylinderOptionsByProduct.set(cylinder.product_id, options);
    });
  });
  ...
  function buildPayload(): CreateMerchandiseReturnPayload | null {
    ...
    const lines = form.lines.map((line) => {
      ...
      if (line.cylinder_id && dispatchCylinderProductById.get(line.cylinder_id) !== orderItem.product_id) {
        return null;
      }
      return {
        ...
        >
          ...
          <div className="space-y-3 rounded-md border border-border p-4">
            ...
            {form.lines.map((line, index) => {
              ...
              const maxQty = Number(orderItem?.received_qty ?? 0) || 0;
              const serialOptions = orderItem?.product_id ? (dispatchCylinderOptionsByProduct.get(orderItem.product_id) ?? []) : [];
                ...
                <div key={index} className="space-y-3 rounded-md border border-border p-3">
                  <div className="grid gap-3 md:grid-cols-2">
                    <label className="block space-y-2 text-sm text-foreground">
                      ...
                      />
                      <p className="text-xs text-muted-foreground">
                        {selectedProduct ? `Producto ${selectedProduct}` : "Selecciona un item para acotar seriales."}
                      </p>
                    </label>
                    <label className="block space-y-2 text-sm text-foreground">
                      <span>Serial</span>
                      <Combobox
                        ...
                        placeholder={orderItem ? (serialOptions.length ? "Seleccionar serial" : "Sin serial disponible") : "Selecciona el item primero"}
                        searchPlaceholder="Buscar serial"
                        disabled={!orderItem || serialOptions.length === 0}
                      />
                      <p className="text-xs text-muted-foreground">{orderItem ? `Disponible ${maxQty}` : "Seriales filtrados por producto de la línea."}</p>
                    </label>
                  </div>
