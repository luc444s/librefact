...
def receive_order(
    ...
    *,
    ...
    for entry in items_payload:
        ...
        serials = [str(serial).strip() for serial in entry.get("serials") or [] if str(serial).strip()]
        ...
        if not serials:
            errors.append(f"Linea {purchase_item_id}: debe incluir seriales de envase")
            continue

        if len(serials) != int(qty):
            errors.append(
                f"Linea {purchase_item_id}: seriales ({len(serials)}) debe igualar recibidas ({int(qty)})"
            )
            continue

        if len(set(serials)) != len(serials):
            errors.append(f"Linea {purchase_item_id}: seriales duplicados en la misma linea")
            continue

        serial_error = False
        for serial in serials:
            if serial in seen_serials:
                errors.append(f"Serial {serial} ya fue asignado a otra linea de la recepcion")
                serial_error = True
                continue
            cylinder = db.scalar(
                select(LogisticsCylinder).where(
            ...
            if cylinder is None:
                errors.append(f"Serial {serial} no encontrado en el tenant")
                serial_error = True
                continue
            if cylinder.product_id != item.product_id:
                errors.append(
                    f"Serial {serial} no pertenece al producto {item.product_id} de la linea"
                )
                serial_error = True
                continue
